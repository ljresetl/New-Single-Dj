(() => {
  // ===========================
  // Scroll reveal (Оптимізовано для фонового запуску)
  // ===========================
  ...
  // Web Audio API змінні (Ініціалізуються ліниво при кліку)
  let ctx = null;
  let analyser = null;
  let dataArray = null;
  ...
  function initAudioContext() {
    if (ctx) return; // Вже ініціалізовано
  ...
  function animate() {
    if (!currentAudio || currentAudio.paused || !analyser) return;
  ...
  function updateTitle(name) {
    ...
    requestAnimationFrame(() => {
      span.classList.toggle(
        "scrolling",
        span.scrollWidth > titleEl.clientWidth,
      );
    });
  }
  ...
  trackItems.forEach((item) => {
    ...
    btn.addEventListener("click", () => {
      const name =
        item.querySelector(".track-name")?.textContent?.trim() || "Unknown";
  ...
  if (audio2 && playBtn2 && progress2 && progCont2 && time2) {
    ...
    progCont2.addEventListener("mousedown", (e) => {
      seeking2 = true;
      seekTo2(e.clientX);
    });
    document.addEventListener("mousemove", (e) => {
      if (seeking2) seekTo2(e.clientX);
    });
    document.addEventListener("mouseup", () => {
      seeking2 = false;
    });

    progCont2.addEventListener(
      "touchstart",
      (e) => {
        seeking2 = true;
        seekTo2(e.touches[0].clientX);
      },
      { passive: true },
    );
    document.addEventListener(
      "touchmove",
      (e) => {
        if (seeking2) seekTo2(e.touches[0].clientX);
      },
      { passive: true },
    );
    document.addEventListener("touchend", () => {
